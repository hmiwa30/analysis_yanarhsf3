...
for(i in c(3,4)){
  for(ah in c("altai","chagyrskaya","denisova","vindija")){
    ...
    resultf3.all <- rbind(resultf3.all,data.frame(resultf3.dat,pops="FIN"))
    #GBR####
    resultf3 <- NULL
    for(part in c(1,2)){
      ( filename <- sprintf("%s_%s_part%s.2.f3_GBR.txt",ft_fix,ah,part) )
      resultf3 <- rbind(resultf3,read.table(filename,header=T))
    }
    resultf3 <- resultf3[-which(resultf3$pop3 %in% c("FIN","JPT","CHB","GBR")),]
    ...
    resultf3.all <- rbind(resultf3.all,data.frame(resultf3.dat,pops="GBR"))
    
    #JPT####
    resultf3 <- NULL
    for(part in c(1,2)){
      ( filename <- sprintf("%s_%s_part%s.2.f3_JPT.txt",ft_fix,ah,part) )
      resultf3 <- rbind(resultf3,read.table(filename,header=T))
    }
    resultf3 <- resultf3[-which(resultf3$pop3 %in% c("FIN","JPT","CHB","GBR")),]
    ...
    resultf3.all <- rbind(resultf3.all,data.frame(resultf3.dat,pops="JPT"))
    
    #OUTPUT####
    resultf3.all$pops <- factor(resultf3.all$pops,levels = c("FIN","GBR","JPT"))
    
    ( outputname <- sprintf("%s_%s_part%s.2.f3_3pop.png",pn_fix,ah,i) )
    ...
    g <- ggplot(resultf3.all,aes(x=f3,y=sample,color=pop,shape=pops)) + 
